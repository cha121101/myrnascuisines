...
<head>
    ...
    <title>Pending Event Reservation Form</title>
...
<body class="bg-orange-50">
    ...
    <!--FORM SECTION-->
        <section>
            <div class="sm:ml-64 print:sm:ml-0" id="form">
                <!--Time & Date-->
                ...
                <!--FORM-->
                    <div class="overflow-hidden bg-opacity-75 max-w-screen">
                        <div class="w-full px-10 mx-auto text-center font-Geologica lg:text-left">
                            <p class="flex px-5 py-5 my-10 text-3xl font-extrabold text-center text-white underline bg-red-700 md:text-4xl xl:text-5xl font-geologica font-Geologica underline-offset-8 decoration-white">Pending Event Reservation Form</p>
                            
                            @forelse($data as $key => $val )
                            <form action="{{ route('action.reservation.event') }}" enctype="multipart/form-data" method="POST" >
                                @csrf
                                <input type="hidden" name="id" value="{{ $val->id }}"> 
                                <input type="hidden" name="customer_id" value="{{ $val->customer_id }}"> 
                                <input type="hidden" name="name" value="{{ Auth::user()->name }}"> 
                                {{-- <input type="hidden" name="reservation_type" value="{{ $val->id }}">  --}}
                                                            ...
                                                            <div class="relative mb-6 text-center">
                                                                <p class="flex text-sm font-extrabold text-left text-black font-nunito input-icon input-icon-right">Time</p> 
                                                                    <div class="relative text-center">
                                                                        <select id="scheduletime" name="scheduletime" onchange="checkscheduledate()" class="bg-gray-50 border text-gray-900 font-geologica font-extrabold text-base rounded-lg focus:ring-blue-500  block w-full p-2.5 dark:placeholder-gray-400 dark:focus:ring-blue-500" >
                                                                            <optgroup>
    ...
    <!--FORM SCRIPT-->
        ...
        //    var date_today = `${date.getMonth()}/${date.getDate()}/${date.getFullYear()}`;
                        ...
                        if(element.x == data.scheduledate && isokay){                            
                            if(isokay){
                                ...
                                if(document.getElementById('scheduledate').value === element.x && element.y === 3 ){
                                  ...
                                  document.getElementById('accepttogglebtn').classList.add('bg-green-600');
                                    document.getElementById('accepttogglebtn').classList.remove('bg-red-600');
                                    document.getElementById('accepttogglebtn').innerHTML = 'Submit';
                                    document.getElementById('schedulebtn').classList.add('hidden')
                                    document.getElementById('accepttogglebtn').classList.remove('hidden')
